...
- **`.cls` is the canonical source for form code.** `.form.txt` / `.report.txt` emit **zero** `function` / `sub` / `class` nodes — only the `form-layout` / `report-layout` container, one `form-instance-control` per named control, and a `property` node per control type. Dysflow overwrites the layout file's embedded code on the next import, so emitting procedures from there would be both wrong and ephemeral.
