...
describe('Issue #261 — adds no rows', () => {
  ...
  it('introduces no node kind and no edge kind of its own', () => {
    ...
    // The kinds below are the ones the VBA extractor already produced before
    // this change; `label` and `handles-error` arrive from #263, which landed
    // separately. The point of this guard is that the error-channel flag adds
    // NOTHING to either set — it only stamps a field on rows that already
    // exist — so it asserts a closed set rather than the absence of one kind.
    const allowedNodeKinds = new Set(result.nodes.map((n) => n.kind));
    expect(allowedNodeKinds.has('variable')).toBe(true);
    for (const node of result.nodes) {
      expect([
        'file', 'module', 'class', 'function', 'variable', 'constant',
        'parameter', 'label', 'property', 'form-layout', 'report-layout',
        'form-instance-control',
      ]).toContain(node.kind);
    }
    for (const edge of result.edges) {
      expect([
        'contains', 'calls', 'references', 'type-member', 'type_of',
        'handles-error', 'event-handler', 'opens-form', 'opens-report',
        'raises-event', 'subscribes-event',
      ]).toContain(edge.kind);
    }
  });
});
