...
describe('CODEGRAPH_DIR override (#636)', () => {
  ...
  describe('isCodeGraphDataDir()', () => {
    ...
    it('matches the legacy .codegraph-vba directory so it is never indexed', () => {
      delete process.env.CODEGRAPH_DIR;
      expect(isCodeGraphDataDir('.codegraph-vba')).toBe(true);
    });
  ...
  it('init writes the index under .codegraph by default, not the legacy .codegraph-vba', () => {
    delete process.env.CODEGRAPH_DIR;
    const cg = CodeGraph.initSync(tempDir);
    try {
