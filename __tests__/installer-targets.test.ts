...
describe('Installer targets — partial-state idempotency', () => {
  ...
  // gentle-ai only recognises `mcp.codegraph` with command
  // ["codegraph","serve","--mcp"] (issue #328). An earlier fork release wrote
  // `mcp["codegraph-vba"]`; a user may also already own `mcp.codegraph` with
  // their own `environment` (e.g. CODEGRAPH_MCP_TOOLS).
  it('opencode: install migrates the legacy codegraph-vba key and keeps the user environment', () => {
    const opencode = getTarget('opencode')!;
    const dir = path.join(tmpHome, '.config', 'opencode');
    fs.mkdirSync(dir, { recursive: true });
    ...
    fs.writeFileSync(file, JSON.stringify({
      $schema: 'https://opencode.ai/config.json',
      mcp: {
        'codegraph-vba': { type: 'local', command: ['codegraph-vba', 'serve', '--mcp'], enabled: true },
        codegraph: {
          type: 'local', command: ['codegraph', 'serve', '--mcp'], enabled: true,
          environment: { CODEGRAPH_MCP_TOOLS: 'explore,callers,impact' },
        },
      },
    }, null, 2));

    expect(opencode.detect('global').alreadyConfigured).toBe(true);
    expect(opencode.install('global', { autoAllow: true }).files[0].action).toBe('updated');

    const mcp = JSON.parse(fs.readFileSync(file, 'utf-8')).mcp;
    expect(mcp['codegraph-vba']).toBeUndefined();
    expect(mcp.codegraph).toEqual({
      type: 'local', command: ['codegraph', 'serve', '--mcp'], enabled: true,
      environment: { CODEGRAPH_MCP_TOOLS: 'explore,callers,impact' },
    });
    expect(opencode.install('global', { autoAllow: true }).files[0].action).toBe('unchanged');
  });

  it('hermes: install migrates the legacy codegraph-vba server and uninstall removes both keys', () => {
    const hermes = getTarget('hermes')!;
    const config = path.join(tmpHome, '.hermes', 'config.yaml');
    fs.mkdirSync(path.dirname(config), { recursive: true });
    fs.writeFileSync(config, [
    ...
    hermes.install('global', { autoAllow: true });
    const installed = fs.readFileSync(config, 'utf-8');
    expect(installed).not.toContain('codegraph-vba');
    expect(installed).toContain('  codegraph:\n    command: codegraph');

    fs.writeFileSync(config, installed.replace('mcp_servers:\n', 'mcp_servers:\n  codegraph-vba:\n    command: codegraph-vba\n'));
    hermes.uninstall('global');
    expect(fs.readFileSync(config, 'utf-8')).not.toMatch(/codegraph(-vba)?:/);
  });
