...
describe('reindexReasons in codegraph status --json (#189)', () => {
  ...
  it('reports currentExtractionVersion=27, reindexRecommended=true, and reindexReasons=[extraction-version] for an index stamped with the previous extraction-version constant (26)', async () => {
    fs.writeFileSync(path.join(tempDir, 'a.ts'), 'export const x = 1;\n');
    const cg = CodeGraph.initSync(tempDir);
    await cg.indexAll();
    cg.close();

    // Simulate an index built by the previous engine (constant 26). The bumped
    // binary (constant 27) must surface the mismatch as `extraction-version` in
    // the reindexReasons array, alongside the boolean reindexRecommended=true.
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const { DatabaseSync } = require('node:sqlite');
    const db = new DatabaseSync(path.join(tempDir, codeGraphDirName(), 'codegraph.db'));
    db.prepare(
      "INSERT INTO project_metadata (key, value, updated_at) VALUES ('indexed_with_extraction_version', '26', 0) " +
    ...
    const out = runStatusJson(tempDir);
    const index = out.index as Record<string, unknown>;
    expect(index.currentExtractionVersion).toBe(27);
    expect(index.builtWithExtractionVersion).toBe(26);
