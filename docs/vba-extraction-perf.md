...
**Date:** 2026-07-18
**Commit:** `c81120177ab6bf5766c15248acf5ddee6fc5250f` (branch `perf/166-vba-extraction-baseline`)
**Engine:** `@aroman22/codegraph-vba` 1.11.0 (post rule-table refactor #153, post timing-instrumentation #156)
**Measurements captured against:** `425e33ad2ba86af1e26279c42ce14fe8cd107589` (the parent of this commit, which is the source tree the timing-instrumented `dist/bin/codegraph.js` was built from when the stderr logs were captured)
...
| Files cited | `ACAuditoriaOperaciones.cls` (472 lines), `ARAuditoria.cls` (476 lines), `Form_FormGestionRiesgos.cls` (372 lines, from bench), `mdlCursor.bas` (39 lines, from bench). |
...
| Raw logs | `C:\Users\adm1\AppData\Local\Temp\opencode\issue-166-bench-runs\run{1,2,3}-*-stderr.log` (kept out-of-tree; not committed). |

## Per-file timing — the four required files

### `ACAuditoriaOperaciones.cls` (472 lines, fixture)

| run | totalMs | preprocess.strip | preprocess.join | preprocess.cc | walk.procedures | walk.main | classifiers.sum |
|---|---|---|---|---|---|---|---|
| run1 (init) | 73.754 | 1.762 | 0.103 | 0.590 | 2.487 | 34.235 | 34.577 |
| run2 (index) | 77.410 | 1.879 | 0.101 | 0.455 | 1.916 | 36.454 | 36.605 |
| run3 (index) | 64.641 | 2.015 | 0.117 | 0.535 | 2.889 | 29.221 | 29.864 |
| **MEDIAN** | **73.754** | **1.879** | **0.103** | **0.535** | **2.487** | **34.235** | **34.577** |
...
### `ARAuditoria.cls` (476 lines, fixture)

| run | totalMs | preprocess.strip | preprocess.join | preprocess.cc | walk.procedures | walk.main | classifiers.sum |
|---|---|---|---|---|---|---|---|
| run1 (init) | 18.499 | 1.038 | 0.049 | 0.364 | 0.735 | 8.505 | 7.808 |
| run2 (index) | 14.926 | 0.856 | 0.029 | 0.207 | 0.521 | 7.079 | 6.234 |
| run3 (index) | 19.326 | 1.243 | 0.036 | 0.260 | 0.529 | 9.102 | 8.156 |
| **MEDIAN** | **18.499** | **1.038** | **0.036** | **0.260** | **0.529** | **8.505** | **7.808** |
...
### `Form_FormGestionRiesgos.cls` (372 lines, bench)

| run | totalMs | preprocess.strip | preprocess.join | preprocess.cc | walk.procedures | walk.main | classifiers.sum |
|---|---|---|---|---|---|---|---|
| run1 (init) | 9.452 | 0.900 | 0.030 | 0.129 | 0.475 | 4.096 | 3.822 |
| run2 (index) | 22.642 | 0.963 | 0.033 | 0.201 | 0.654 | 10.854 | 9.937 |
| run3 (index) | 23.830 | 1.250 | 0.044 | 0.173 | 0.557 | 11.350 | 10.456 |
| **MEDIAN** | **22.642** | **0.963** | **0.033** | **0.173** | **0.557** | **10.854** | **9.937** |
...
### `mdlCursor.bas` (39 lines, bench + fixture)
...
The required files are small (≤476 lines). The bench corpus contains much larger files where the rule-table overhead would be most visible if it scaled super-linearly. For context:

| File | Lines | Median total (ms) | Median ms/line | walk.main share |
|---|---|---|---|---|
| `Funciones Generales.bas` | 9 535 | 303.0 | 0.032 | 46% |
| `Constructor.bas` | 7 609 | 217.6 | 0.029 | 44% |
| `Riesgo.cls` | 7 172 | 225.5 | 0.031 | 47% |
| `Edicion.cls` | 5 597 | 235.2 | 0.042 | 46% |
| `Proyecto.cls` | 4 882 | 239.5 | 0.049 | 44% |

Amortized cost stays in the **30–50 μs/line** band across the largest files. `Form_FormGestionRiesgos.cls` at 372 lines lands at **61 μs/line** (median) — the worst amortized cost in the table — but its absolute time (22.6ms) is still negligible against the corpus-level budget. The `preprocess.cc.lexer+parser` inner stage stays at 0.000ms on all of these files (no `#If`/`#Const` directives).
...
| File | Lines | Median total | Median ms/line |
|---|---|---|---|
| `ACAuditoriaOperaciones.cls` | 472 | 73.754 | 0.156 |
| `ARAuditoria.cls` | 476 | 18.499 | 0.039 |
| `Form_FormGestionRiesgos.cls` | 372 | 22.642 | 0.061 |
| `mdlCursor.bas` | 39 | 3.736 | 0.096 |

**Corpus-level median:** 6 437 ms for 248 `.cls`/`.bas` files, 11.5k nodes, 24.5k edges. ~26 ms/file, ~14 μs/line on average.
...
```powershell
# 1. Build the engine from this worktree's tip.
...
# 4. Parse the per-file blocks (the script is committed to this issue's branch — see parse-timings.mjs in the worktree).
node parse-timings.mjs > summary.txt
```
