...
import { buildNodeTooOldBanner, isBelowMinimumNodeVersion } from './node-version-check';
...
async function loadCodeGraph(): Promise<typeof import('../index')> {
  try {
    ...
    console.error(`\n  Error: ${msg}`);
    const { cliInstallCommand } = await import('../upgrade');
    console.error(`\n  Try reinstalling with: ${cliInstallCommand(process.platform)}\n`);
    process.exit(1);
  }
}
...
if (isBelowMinimumNodeVersion(nodeVersion)) {
